...
import { KeyIcon, LockClosedIcon, UserPlusIcon } from "@heroicons/react/24/solid";
...
const AuthDialog: FC<AuthDialogProps> = ({
    ...
    const [authApiErrorMessage, setAuthApiErrorMessage] = useState<string | null>(null);
    ...
    const onLoginError = (error: any) => {
        ...
        switch (error.code) {
            ...
            case AuthApiErrors.EMAIL_VERIFICATION_REQUIRED.code:
                setAuthApiErrorMessage(AuthApiErrors.EMAIL_VERIFICATION_REQUIRED.message);
                break;
            default:
                setAuthApiErrorMessage("An error occured while logging in. Please try again later.");
                break;
        }
    }
    ...
    const onSignupError = (error: any) => {
        ...
        switch (error.code) {
            ...
            case AuthApiErrors.EMAIL_ALREADY_IN_USE.code:
                ...
                break;
            default:
                setAuthApiErrorMessage("An error occured while signing up. Please try again later.");
                break;
        }
    }
                            ...
                            >
                                <Dialog.Panel className="flex w-full max-w-md transform flex-col divide-y divide-gray-200 overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                        ...
                                        <h3 className="flex flex-row items-center gap-x-1.5 text-base font-semibold leading-6 text-gray-900">
                                            {
                                                authDialogState === AuthDialogState.LOGIN
                                                    ...
                                                    :
                                                    authDialogState === AuthDialogState.FORGOT_PASSWORD
                                                        ?
                                                        <>
                                                        ...
                                                        :
                                                        <>
                                                            <UserPlusIcon className="h-7 w-7 text-gray-400" />
                                                            Create an account
                                                        </>
                                    ...
                                    <div>
                                        {
                                            authApiErrorMessage && (
                                                <div className="mt-3 flex flex-col items-center justify-center gap-y-2 bg-red-100 rounded-lg p-4 shadow-lg">
                                    ...
                                    <div>
                                        {
                                            isAuthenticating && (
                                                <div className="absolute inset-0 flex items-center justify-center backdrop-filter backdrop-blur-sm bg-black bg-opacity-50">
                                                    <div className="flex flex-col items-center justify-center gap-y-2 bg-white rounded-lg p-4 shadow-lg">
                                                        ...
                                                        <p className="text-blue-500 dark:text-white">Please wait...</p>
                                                    </div>
                                                </div>
                                            )
                                        }
